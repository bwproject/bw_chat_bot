...
def create_client(session_data=None):
    ...
    if MTPROXY_HOST and MTPROXY_PORT and MTPROXY_SECRET:
        kwargs["proxy"] = (MTPROXY_HOST, int(MTPROXY_PORT), MTPROXY_SECRET)
        kwargs["connection"] = ConnectionTcpMTProxyRandomizedIntermediate

    # StringSession or file session
...
async def send_to_user(user_id, text):
    ...
    try:
        await client.send_message(reply_map[user_id], text)
        return True

    except Exception as e:
        logger.error(f"send_to_user error: {e}")
        return False
...
async def create_new_chat(username, text):
    ...
    try:
        ...
        logger.error(f"create_new_chat error: {e}")
        return None
